...
bot = Bot(token="7437528815:AAE1TXgmakbWja3Ar7QuTjpF_Xpm77L5-D8")

dp = Dispatcher(bot)

print('ProfDetali Bot')
...
async def send_welcome(message):
    ...
    if str(adm_id) == str(message.from_user.id):
        ...
        inline_btn_3 = InlineKeyboardButton('📌Добавить Подсветку', callback_data='add_film')
        ...
        inline_btn_7 = InlineKeyboardButton('📌Удалить адресс подсветки', callback_data='delete_number')
...
async def send_welcome(message):
    ...
    if len(channels_me) == 0:
        ...
        for i in channels_me:
            ...
            if user_channel_status["status"] == 'left':
                ...
                for i in channels_me:
                    inline_btn_1 = InlineKeyboardButton(str(channels_me[i]), url='https://t.me/profdetaliverf'+ str(i))
    ...
    if c == 1:
       ...
       await bot.send_message(message.from_user.id, text = '*Привет это ProfDetali- БОТ | Поиск🔍\nЧтобы узнать где лежит подсветка.\nНажмите на «🔎Поиск» 👇*',parse_mode = 'Markdown',reply_markup = start.greet_kb1)
...
async def vote_up_cb_handler(call):
    ...
    await bot.send_message(call.from_user.id, text = '*Введите номер подсветки, которую необходимо удалить*',parse_mode = 'Markdown')
...
async def vote_up_cb_handler(call):
    ...
    await bot.send_message(call.from_user.id, text = '*Введите артикул подсветки*',parse_mode = 'Markdown')
...
async def vote_up_cb_handler(call):
    ...
    if c == 1:
       ...
       await bot.send_message(call.from_user.id, text = '*Привет это ProfDetali- БОТ | Поиск🔍\nЧтобы узнать где лежит подсветка.\nНажмите на «🔎Поиск» 👇*',parse_mode = 'Markdown',reply_markup = start.greet_kb1)
...
async def send_welcome(message):
    ...
    if c == 1:
        ...
        if str(message.text) == '🔎Поиск':
            a = 1
            await bot.send_message(chat_id = message.from_user.id,text = '*🔎 Для поиска отправьте Артикул без приставки "LED-"*',parse_mode = 'Markdown')
        if user['quest'] == 1 and a == 0:
            ...
            if str(message.text) in q:
                ...
                await bot.send_message(chat_id = message.from_user.id, text = '*Адрес полки:*',parse_mode = 'Markdown')
        ...
        if user['quest'] == 3 and a == 0:
            ...
            await bot.send_message(chat_id = message.from_user.id, text = '*Введите ссылку на фотографию подсветки*',parse_mode = 'Markdown')
        ...
        if user['quest'] == 992 and a == 0:
            ...
            await bot.send_message(chat_id = message.from_user.id, text = '*Подсветка успешно добавлена!*',parse_mode = 'Markdown')
        ...
        if a == 0:
            ...
            if str(message.text) in q:
                try:
                    await bot.send_photo(chat_id = message.from_user.id, photo = str(q[str(message.text)][0]),caption = '*Адрес полки "' + q[str(message.text)][1] + '"*',parse_mode = 'Markdown')
                except Exception as ex:
                    print(ex)
